...
export default function App() {
  ...
  const filteredProperties = useMemo(() => filterAndSortProperties(properties, filters), [properties, filters]);
  ...
  return <main>
    <section className="hero"><nav><a className="brand" href="#top"><span>H</span> HAVEN</a><div className="nav-links"><a href="#properties">Buy</a><a href="#properties">Sell</a><a href="#intelligence">Intelligence</a><a href="#about">Our story</a></div><AuthControls /><a className="nav-cta" href="#intelligence">Speak with an advisor <MoveRight size={17} /></a><button className="mobile-menu" type="button" aria-label="Open menu"><Menu size={23} /></button></nav><div className="hero-content" id="top"><div className="eyebrow"><Sparkles size={15} /> Curated homes. Human guidance.</div><h1>Find your next <em>sanctuary.</em></h1><p>Discover exceptional homes, thoughtfully matched to the life you want to live.</p><form className="search-panel" onSubmit={submitSearch}><label><MapPin size={19} /><span>Location<input value={draftQuery} onChange={(event) => setDraftQuery(event.target.value)} placeholder="City, neighborhood, or listing" aria-label="Search Bataan listings" /></span></label><label><span className="search-label-icon">$</span><span>Price range<select value={filters.priceBand} onChange={(event) => updateFilter('priceBand', event.target.value)} aria-label="Price range"><option value="">Any price</option><option value="under1m">Under ₱1M</option><option value="1m-5m">₱1M–₱5M</option><option value="5m-10m">₱5M–₱10M</option><option value="over10m">Over ₱10M</option></select></span><ChevronDown size={18} /></label><label><Building2 size={19} /><span>Property type<select value={filters.propertyType} onChange={(event) => updateFilter('propertyType', event.target.value)} aria-label="Property type"><option value="">All properties</option><option>House</option><option>Condo</option><option>Land</option><option>Commercial</option></select></span><ChevronDown size={18} /></label><button type="submit"><Search size={19} /> Search homes</button></form></div><div className="hero-foot"><span><i /> New homes matched daily</span><span>Trusted by 12,000+ home seekers</span></div></section>
    <section className="properties" id="properties"><div className="section-heading"><div><p className="section-kicker">Bataan listings</p><h2>Homes worth coming home to.</h2></div><label className="sort-control">Sort by<select value={filters.sort} onChange={(event) => updateFilter('sort', event.target.value)}><option value="newest">Newest</option><option value="price-asc">Price: low to high</option><option value="price-desc">Price: high to low</option><option value="title">Listing title</option></select></label></div>{!loading && !error && <div className="results-toolbar"><p className="result-count">{filteredProperties.length} listing{filteredProperties.length === 1 ? '' : 's'} found{savedIds.size ? ` · ${savedIds.size} saved` : ''}</p>{hasActiveFilters && <button className="clear-search" type="button" onClick={clearSearch}>Clear search</button>}</div>}{loading && <div className="property-state">Loading Bataan property listings…</div>}{error && <div className="property-state error-state"><p>{error}</p><button type="button" onClick={() => void loadProperties()}>Try again</button></div>}{!loading && !error && visibleProperties.length === 0 && <div className="property-state">No listings match those filters. <button className="clear-search" type="button" onClick={clearSearch}>Clear search</button></div>}{!loading && !error && visibleProperties.length > 0 && <><div className="property-grid">{visibleProperties.map((property) => <PropertyCard property={property} key={property.id} onSelect={setSelected} isSaved={savedIds.has(property.id)} onToggleSaved={toggleSaved} />)}</div><nav className="pagination" aria-label="Property listing pages"><button type="button" disabled={page === 1} onClick={() => setPage((current) => current - 1)}>Previous</button><span>Page {page} of {pageCount}</span><button type="button" disabled={page === pageCount} onClick={() => setPage((current) => current + 1)}>Next</button></nav></>}</section>
