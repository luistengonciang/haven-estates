-- Make municipality terms hard filters instead of weak lexical ranking hints.
...
as $$
  with query as (select websearch_to_tsquery('simple', trim(search_text)) as terms), candidates as (
    select property.*,
  ...
  select candidates.id, candidates.title, candidates.price, candidates.location, candidates.bedrooms, candidates.bathrooms, candidates.floor_area, candidates.source_url,
  ...
  where query.terms <> ''::tsquery
    ...
    and (property_type is null or lower(concat_ws(' ', candidates.title, candidates.location, candidates.source_url)) ~ '(house|villa|townhouse)-for-sale|house and lot|house for sale|type: h & l|bungalow|[0-9]+br')
    and (location_filter is null or lower(concat_ws(' ', candidates.title, candidates.location, candidates.source_url)) like '%' || lower(location_filter) || '%')
  order by rank desc, candidates.scraped_at desc nulls last
  limit greatest(1, least(coalesce(match_count, 3), 10));
$$;

revoke execute on function public.search_bataan_properties(text, integer, numeric, text, text) from public, anon;
grant execute on function public.search_bataan_properties(text, integer, numeric, text, text) to authenticated;
